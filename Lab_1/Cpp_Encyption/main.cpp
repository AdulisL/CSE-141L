...
byte reduct_xor(char  input){
    char temp1 = input >> 4;
    char temp2 = input ^ temp1;
    char temp3 = temp2 >> 2;
    char temp4 = temp3 ^ temp2;
    char temp5 = temp4 >> 1;
...
//        lfsr_state[i+1] = lfsr_function(lfsr_state[i]);
        lfsr_state[i + 1] = (lfsr_state[i] << 1) |
                reduct_xor((lfsr_state[i]&LFSR_ptrn[0]));
    }
...
int main() {
    ...
    // Testing for reduction xor
    char *input = "ABCDEFG";
    for (int i = 0; i < 7; i++) {
        encrypted[i] = reduct_xor(*input++);
    }
